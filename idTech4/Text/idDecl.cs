	...
	public class idDecl : IDisposable
	{
		...
		public bool IsImplicit
		{
			get
			{
				...
				return (this.SourceFile == idEngine.Instance.GetService<IDeclManager>().ImplicitDeclFile);
			}
		}
		...
		public void MakeDefault()
		{
			...
			idEngine.Instance.GetService<IDeclManager>().MediaPrint("DEFAULTED");
			...
			{
				idEngine.Instance.FatalError("make default: bad defaultDefinition(): {0}", defaultText);
				return;
			}
		...
		internal void ParseLocal()
		{
			...
			declManager.MediaPrint("parsing {0} {1}", this.Type.ToString().ToLower(), this.Name);
