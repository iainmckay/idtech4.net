...
namespace idTech4.Text.Decls
{
	public class idDeclParticle : idDecl
	{
		...
		private idParticleStage ParseParticleStage(idLexer lexer)
		{
			...
			while(true)
			{
				...
				else
				{
					...
					{
						token = lexer.ReadToken();
						stage.Material = declManager.FindMaterial(token.ToString());
					}
		...
		private void ParseParametric(idLexer lexer, idParticleParameter parm)
		{
			...
			{
				parm.Table = (idDeclTable) idEngine.Instance.GetService<IDeclManager>().FindType(DeclType.Table, token.ToString(), false);
			}
		}
		...
		public override int MemoryUsage
		{
			get
			{
				idLog.Warning("TODO: idDeclParticle.MemoryUsage");
				return 0;
			}
		}
		#endregion
		...
		public override bool Parse(string text)
		{
			...
			{
				idLog.Warning("TODO: GetStageBounds");
	...
	public sealed class idParticleStage
	{
		...
		public void Default()
		{
			_material = idEngine.Instance.GetService<IDeclManager>().FindMaterial("_default");
			_totalParticles = 100;
			_spawnBunching = 1.0f;
			_particleLife = 1.5f;
			_timeOffset = 0.0f;
